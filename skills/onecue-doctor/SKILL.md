---
name: onecue-doctor
description: Diagnoses whether OneCue project memory is set up correctly in the current repository. This skill should be used when the user asks to "check OneCue", "run onecue doctor", "is OneCue working", "debug OneCue", or when OneCue decisions do not seem to be remembered or recalled. It verifies the local store, the Claude Code hook installation, and the Node runtime, then suggests the exact fix for anything that is wrong. Do not use it to create or recall decisions — that is the onecue skill.
license: MIT
metadata:
  author: onecueai
  version: "0.1.0"
...
**Check the setup before blaming the store.**

OneCue doctor verifies that project decisions can actually work in this repository: the store directory, the Claude Code hooks, and the runtime. It also reports observed impact — prompts seen, cues surfaced, decision reuse — counted from local session logs. Run it when something feels off — a decision was not recalled, a hook seems silent, or the user is unsure OneCue is installed.
...
2. Otherwise run the bundled script — same checks, zero install, exits non-zero on warnings:
   ...
   The script checks the canonical git-visible store (`.onecue/decisions/` in
   the project), the skill-only fallback (`.onecue/memories/`), and CLI session
   logs (`~/.onecue/projects/<fingerprint>/`).
...
- `decision store present` — `.onecue/decisions/` exists, or the CLI is initialized, or the skill-only fallback `.onecue/memories/` exists.
- `store writable` — the store location can be written.
- `settings readable (claude|devin)` — the harness settings file parses.
- `hooks (claude|devin)` — OneCue hooks exist for the expected events (UserPromptSubmit, PreToolUse, PostToolUse, Stop, SessionEnd).
- `hook targets exist` — hook commands point at a CLI path that still exists.
- `decisions readable` — `.onecue/decisions/*.md` files are present and named as decision files.
- `skill memories imported` — with the CLI installed, no leftover files in `.onecue/memories/`.
...
3. **Initialization.** `.onecue/decisions/` exists (CLI) or `.onecue/memories/` exists (skill-only). An empty directory is fine; a missing one is not.
...
5. **Hooks.** Each existing settings file must contain OneCue hook entries (look for `onecue` in the hooks section). Hooks are what let decisions surface automatically.
...
- Store missing/not writable → fix permissions, then `onecue init` (or create `.onecue/decisions/` — or `.onecue/memories/` if there is no CLI).
...
- Never create or modify decisions — doctor only diagnoses.
